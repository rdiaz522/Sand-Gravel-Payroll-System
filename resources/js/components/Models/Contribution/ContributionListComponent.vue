<template>
    <div>
        <div class="card shadow mb-4">
            ...
            <datatable
                title="Employee Contribution List"
                :columns="columns" 
                ...
                <th slot="thead-tr">
                  ...
                  <template slot="tbody-tr" slot-scope="props">
                    <td>
                      ...
                      <button class="btn btn-success btn-sm" v-on:click="(e) => onShowOverTotal(props.row, e)">
                        Overall Total
                      </button>
                    </td>
                  </template>
...
import DataTable from "vue-materialize-datatable";
    export default {
        ...
        methods: {
            onEdit(props,row,e) {
                this.$emit('editContribution',props);
            },
            onShowOverTotal(props, row, e) {
                this.$emit('showOverTotal',props);
            },
            ...
            onDelete(props,row,e) {
                 this.$WARNING_MESSAGE.fire({
                    ...
                    if (result.isConfirmed) {
                        this.$SHOW_LOADING();
                        axios.delete(this.$BASE_URL + this.$CONTRIBUTION + `/${props.id}`).then((response) =>  {
                            this.$parent.getContributions();
                            this.$parent.showContributionEdit = false;
                            this.$HIDE_LOADING();
                            this.$WARNING_MESSAGE.fire(
                            'Deleted!',
